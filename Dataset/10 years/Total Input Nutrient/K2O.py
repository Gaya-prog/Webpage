...
years = [f"Y{year}" for year in range(2012, 2023)]
...
filtered_data = filtered_columns[
    ...
    (filtered_columns["Item"].isin(["Nutrient potash K2O (total)"]))
]
...
if not filtered_data.empty:
    output_file_path = os.path.join(output_dir, "FilteredData_Min9Years_K2Ototal.csv")
