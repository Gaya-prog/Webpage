...
# Load your dataset
data <- read.csv("C:/Users/mgaya/OneDrive/Documents/Nanofertech/Webpage/Dataset/10 years/Production/ProductionK20_total.csv")

# Filter data for the first 50 countries
fourth_50_countries <- unique(data$Area)[151:200]  # Get the first 50 unique countries
data_filtered <- data %>%
...
# Generate a custom dark color palette for the countries
...
plot <- ggplot(data_summary, aes(x = Year, y = Total, fill = Area)) +
  ...
  labs(
    title = "Total Nutrient K20 for Production, Import and Export Quantity: {closest_state}",
    x = "Year",
    y = "Total Value",
    fill = "Country"
  ) +
...
anim_save("C:/Users/mgaya/OneDrive/Documents/Nanofertech/Webpage/images/Production/Nutrient_K20_4.gif", animation = animated_plot)
